import torch
import pandas as pd
import numpy as np
from src.meta.transformer_weight import MetaTransformer

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

def meta_transformer_strategy(df: pd.DataFrame,
                              model_path="models/meta_transformer.pt",
                              temperature=1.0,
                              **kwargs):

    # 加载 checkpoint
    ckpt = torch.load(model_path, map_location=DEVICE)
    state_dict = ckpt["state_dict"]
    factor_cols = ckpt["factor_cols"]
    strat_cols = ckpt["strat_cols"]
    seq_len = ckpt["seq_len"]

    input_dim = len(factor_cols)        # = 11
    output_dim = len(strat_cols)        # = 4

    # 构建模型（必须和 checkpoint 一致）
    model = MetaTransformer(
        input_dim=input_dim,
        hidden_dim=64,
        n_heads=4,
        n_layers=2,
        output_dim=output_dim
    ).to(DEVICE)

    model.load_state_dict(state_dict)
    model.eval()

    # ====== 组输入 X ======
    df = df.copy()
    X = torch.tensor(df[factor_cols].values, dtype=torch.float32)

    if len(X) < seq_len:
        df["signal"] = 0
        return df

    preds = []

    for i in range(seq_len, len(X)):
        seq = X[i - seq_len:i].unsqueeze(0).to(DEVICE)
        with torch.no_grad():
            w = model(seq)   # shape: (1, 4)

        w = torch.softmax(w / temperature, dim=-1).cpu().numpy()[0]  # 权重
        preds.append(w)

    preds = np.array(preds)         # (T, 4)
    S = df[strat_cols].iloc[seq_len:].values  # (T, 4)

    # Weighted combination
    out_sig = (preds * S).sum(axis=1)

    signals = np.zeros(len(df))
    signals[seq_len:] = out_sig
    df["signal"] = signals

    return df
